...
const EnhancedGroupChat = ({ groupId, currentUser, groupMembers = [] }) => {
  ...
  const createDefaultGroupChat = async () => {
    try {
      ...
      const participantIds = groupMembers.map(member => member.user?.id || member.user_id).filter(id => id && id !== currentUser?.id)
  ...
  const loadMessages = async (chatId, before = null) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/enhanced-chat/chats/${chatId}/messages?${params}`, {
        headers: {
          Authorization: `Bearer ${session.access_token}`
        }
      })
  ...
  const handleNewMessage = useCallback(
    message => {
      ...
      setChats(prev => prev.map(chat => (chat.id === message.chat_id ? { ...chat, last_message_at: message.created_at } : chat)))
    },
    [activeChat]
  )
  ...
  const handleReactionRemoved = useCallback(({ messageId, userId, emoji }) => {
    setMessages(prev =>
      prev.map(msg => {
        if (msg.id === messageId) {
          const filteredReactions = (msg.message_reactions || []).filter(r => !(r.user_id === userId && r.emoji === emoji))
          return {
            ...msg,
    ...
    for (const file of files) {
      try {
        ...
        if (response.ok) {
          ...
          const uploadResponse = await fetch(`${API_BASE_URL}/api/enhanced-chat/chats/${activeChat.id}/messages/${message.id}/attachments`, {
            method: "POST",
            headers: {
              Authorization: `Bearer ${session.access_token}`
            },
  ...
  const editMessage = async (messageId, newContent) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/enhanced-chat/chats/${activeChat.id}/messages/${messageId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session.access_token}`
  ...
  const searchMessages = async query => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/enhanced-chat/chats/${activeChat.id}/search?${params}`, {
        headers: {
          Authorization: `Bearer ${session.access_token}`
        }
      })
  ...
  const getUserName = user => {
    return user?.raw_user_meta_data?.username || user?.raw_user_meta_data?.full_name || user?.email?.split("@")[0] || "Unknown User"
  }

  const getUserAvatar = user => {
    return user?.raw_user_meta_data?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(getUserName(user))}&background=random`
  }
    ...
    <div className="enhanced-group-chat">
      ...
      <div className="chat-header">
        <div className="chat-info">
          ...
          <div className="chat-status">{typingUsers.size > 0 && <span className="typing-indicator">{Array.from(typingUsers).length === 1 ? "Someone is typing..." : `${typingUsers.size} people are typing...`}</span>}</div>
        </div>

        <div className="chat-actions">
          <button className="search-btn" onClick={() => setShowSearch(!showSearch)} title="Search messages">
            🔍
          </button>
          <button className="call-btn" onClick={() => initiateCall("audio")} title="Audio call">
            📞
          </button>
          <button className="video-call-btn" onClick={() => initiateCall("video")} title="Video call">
            📹
          </button>
        </div>
      </div>
      ...
      {activeCall && (
        <div className="active-call-overlay">
          <div className="call-info">
            <h3>
              {activeCall.type === "video" ? "📹" : "📞"} {activeCall.type} Call
            </h3>
      ...
      >
        {loadingMessages && <div className="loading-more">Loading more messages...</div>}

        {messages.map((message, index) => {
          const prevMessage = messages[index - 1]
          const showDateSeparator = !prevMessage || formatDate(message.created_at) !== formatDate(prevMessage.created_at)
          const isOwnMessage = message.sender_id === currentUser.id
          const showAvatar = !prevMessage || prevMessage.sender_id !== message.sender_id || new Date(message.created_at) - new Date(prevMessage.created_at) > 300000 // 5 minutes

          return (
            <React.Fragment key={message.id}>
              {showDateSeparator && <div className="date-separator">{formatDate(message.created_at)}</div>}

              <div className={`message ${isOwnMessage ? "own-message" : "other-message"}`}>
                {!isOwnMessage && showAvatar && <img src={getUserAvatar(message.sender)} alt={getUserName(message.sender)} className="message-avatar" />}

                <div className="message-content">
                  {!isOwnMessage && showAvatar && <div className="message-sender">{getUserName(message.sender)}</div>}

                  {message.reply_to_id && <div className="reply-reference">Replying to: {message.reply_to?.content}</div>}
                            ...
                            {message.message_attachments.map(attachment => (
                              <div key={attachment.id} className="attachment">
                                {attachment.mime_type.startsWith("image/") ? (
                                  <img src={attachment.file_url} alt={attachment.original_filename} className="attachment-image" />
                                ) : (
                  ...
                  <div className="message-actions">
                    <button className="action-btn" onClick={() => setShowEmojiPicker(message.id)} title="Add reaction">
                      😊
                    </button>
                    <button className="action-btn" onClick={() => setReplyingTo(message)} title="Reply">
                      ↩️
                    </button>
                    {isOwnMessage && (
                      <>
                        ...
                        <button className="action-btn" onClick={() => deleteMessage(message.id)} title="Delete">
                          🗑️
                        </button>
                      </>
                    )}
                  ...
                  {showEmojiPicker === message.id && (
                    <div className="emoji-picker">
                      {commonEmojis.map(emoji => (
                        <button key={emoji} className="emoji-btn" onClick={() => addReaction(message.id, emoji)}>
                          {emoji}
                        </button>
                      ))}
                    </div>
      ...
      {replyingTo && (
        <div className="reply-preview">
          <div className="reply-content">Replying to: {replyingTo.content}</div>
      ...
      {editingMessage && (
        <div className="edit-preview">
          <div className="edit-content">Editing message</div>
          <button
      ...
      <form className="message-input-form" onSubmit={sendMessage}>
        <input type="file" ref={fileInputRef} multiple style={{ display: "none" }} onChange={e => handleFileUpload(Array.from(e.target.files))} />

        <button type="button" className="attach-btn" onClick={() => fileInputRef.current?.click()} title="Attach file">
          📎
        </button>

        <input ref={messageInputRef} type="text" value={newMessage} onChange={handleTyping} placeholder={editingMessage ? "Edit message..." : "Type a message..."} className="message-input" />
