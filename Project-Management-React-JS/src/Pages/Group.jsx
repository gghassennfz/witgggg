...
const AddTaskForm = ({ onTaskCreate }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="add-task-form">
      <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="New task title..." required />
...
const Group = () => {
  ...
  const fetchGroupDetails = async () => {
    ...
    try {
      ...
      // Fetch projects
      const { data: projectRows, error: projectError } = await supabase.from("projects").select("*").eq("group_id", groupId)
  ...
  const handleInvite = async email => {
    try {
      ...
      if (!currentMembers.some(m => m.user_id === user.id)) {
        ...
        const { error: updateError } = await supabase.from("groups").update({ members: updatedMembers }).eq("id", groupId)
      ...
      <div className="members-tab">
          ...
          {members.length > 0 ? (
            members.map(m => (
              <li className="member-row" key={m.user?.id || m.user_id}>
                <img className="avatar" src={m.user?.avatar_url || "/default-avatar.png"} alt={m.user?.username || "User"} style={{ background: "#eee", border: "1px solid #ccc" }} />
                <span className="member-name" style={{ color: "#222" }}>
        ...
        <div style={{ marginTop: 32, background: "#f7faff", padding: "1.2rem 1.5rem", borderRadius: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.03)" }}>
              ...
              {eligibleInvitees.map(mate => (
                <li className="member-row" key={mate.mateInfo?.id || mate.id}>
                  <span style={{ color: "#222" }}>
                    {mate.mateInfo?.username || mate.username} <span style={{ color: "#888" }}>(#{mate.mateInfo?.code || mate.code})</span>
                  </span>
                  <button className="remove-btn" style={{ background: "#27ae60", marginLeft: 12 }} onClick={() => handleInvite(mate.mateInfo?.email || mate.email)}>
                    Invite
                  </button>
                </li>
              ))}
          ...
          <form
            ...
            style={{ marginTop: 18 }}
          >
            <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="Invite by email..." required style={{ color: "#222", background: "#fff", border: "1px solid #bbb" }} />
            <button type="submit" style={{ background: "#007bff", color: "#fff", fontWeight: 500 }}>
              Invite
            </button>
          </form>
        </div>
  ...
  function ProjectsTab() {
    ...
    return (
      <div className="projects-tab">
              ...
              <div key={project.id} className="project-card">
                ...
                <div className="project-content">
                  ...
                  <div className="project-members">
                    <span className="members-count">{members.length} member(s)</span>
                  </div>
                </div>

                <div className="project-actions">
                  <button className="btn btn-primary btn-sm" onClick={() => navigate(`/group/${groupId}/project/${project.id}`)}>
                    Open Project
                  </button>
                </div>
              </div>
        ...
        {showCreateModal && <CreateProjectModal onClose={() => setShowCreateModal(false)} onSubmit={handleCreateProject} />}
      </div>
    )
  }
  ...
  function ChatTab() {
    return (
      <div className="chat-tab-container">
        <EnhancedGroupChat groupId={groupId} currentUser={chatUser} groupMembers={members} />
      </div>
    )
  }
    ...
    <>
      <div className="group-chat" style={{ maxWidth: 700, margin: "0 auto", background: "#fff", borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", padding: 0 }}>
        <div className="chat-messages" style={{ maxHeight: 400, overflowY: "auto", padding: "24px 18px 12px 18px", background: "#f8fafd", borderRadius: "12px 12px 0 0" }}>
          ...
          {(() => {
            ...
            return groups.map((group, gi) => (
              <div key={gi + "-" + group.senderId} className={"chat-message-group" + (group.senderId === currentUserId ? " chat-message-group-own" : "")}>
                <div className="chat-message-group-header">
                  <img className="chat-avatar" src={group.sender.avatar} alt={group.sender.username} />
                ...
                <div className="chat-message-bubbles">
                  {group.messages.map((msg, mi) => {
                    ...
                    return (
                      <div key={mi} className={"chat-bubble" + (group.senderId === currentUserId ? " chat-bubble-own" : "")}>
                        ...
                        {msg.file &&
                          (isImage ? (
                            <div style={{ marginTop: 6 }}>
                              ...
                              <a href={msg.file.data} download={msg.file.name} style={{ fontSize: 13, color: "#007bff", display: "block", marginTop: 4 }}>
                                Download Image
                              </a>
                            </div>
                          ) : (
                            <div style={{ marginTop: 6 }}>
                              <a href={msg.file.data} download={msg.file.name} style={{ fontSize: 14, color: "#007bff" }}>
                                File: {msg.file.name}
                              </a>
                            </div>
                          ))}
        ...
        <form className="chat-input-form" onSubmit={sendMessage} style={{ display: "flex", gap: 10, padding: "16px 18px", borderTop: "1px solid #f0f0f0", background: "#f8fafd", borderRadius: "0 0 12px 12px" }}>
          <input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Type a message..." style={{ flex: 1, padding: "10px 14px", borderRadius: 8, border: "1px solid #ddd", fontSize: 15 }} />
          <input type="file" onChange={e => setFile(e.target.files[0])} style={{ width: 120 }} />
          <button type="submit" style={{ background: "#007bff", color: "#fff", border: "none", borderRadius: 8, padding: "0 18px", fontWeight: 500, fontSize: 15 }}>
            Send
          </button>
        </form>
        ...
        <div style={{ display: "flex", gap: 10, margin: "18px 0 0 0", flexWrap: "wrap", alignItems: "center" }}>
          {otherMembers.map(m => (
            <div key={m.user?.id || m.user_id} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <button onClick={() => setCallModal({ open: true, targetUser: m.user?.id || m.user_id, video: false })} style={{ background: "#27ae60", color: "#fff", border: "none", borderRadius: 6, padding: "4px 10px", marginLeft: 4, fontSize: 14, cursor: "pointer" }}>
                Call
              </button>
              <button onClick={() => setCallModal({ open: true, targetUser: m.user?.id || m.user_id, video: true })} style={{ background: "#007bff", color: "#fff", border: "none", borderRadius: 6, padding: "4px 10px", marginLeft: 4, fontSize: 14, cursor: "pointer" }}>
                Video
              </button>
            </div>
          ))}
      ...
      {/* Call Modal */}
      <GroupCallModal isOpen={callModal.open} onClose={() => setCallModal({ open: false, targetUser: null, video: false })} targetUser={callModal.targetUser} currentUserId={currentUserId} video={callModal.video} groupId={groupId} />
    </>
  )
}
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-body">
          <div className="form-group">
            ...
            <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Enter project name..." required />
          </div>

          <div className="form-group">
            <label>Description</label>
            <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Describe your project..." rows="3" />
          </div>
          ...
          <div className="form-group">
            ...
            <input type="url" name="github_repo" value={formData.github_repo} onChange={handleChange} placeholder="https://github.com/username/repo" />
          </div>
