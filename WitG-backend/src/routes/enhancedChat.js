...
router.post("/chats", async (req, res) => {
  try {
    ...
    // Add participants
    const participants = [{ chat_id: chat.id, user_id: userId, role: "admin" }, ...(participant_ids || []).map(id => ({ chat_id: chat.id, user_id: id, role: "member" }))]
...
router.get("/chats/:chatId/messages", async (req, res) => {
  try {
    ...
    const { data: participant } = await supabase.from("chat_participants").select("id").eq("chat_id", chatId).eq("user_id", userId).single()
  ...
  try {
    ...
    const { data: message, error: messageError } = await supabase
      ...
      .select(
        `
        *,
        sender:users!messages_sender_id_fkey(id, email, raw_user_meta_data)
      `
      )
      .single()
...
router.post("/chats/:chatId/messages/:messageId/attachments", upload.single("file"), async (req, res) => {
  try {
    ...
    const { data: message } = await supabase.from("messages").select("sender_id").eq("id", messageId).eq("chat_id", chatId).eq("sender_id", userId).single()
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage.from("chat-files").upload(filePath, file.buffer, {
  ...
  try {
    ...
    const { data: reaction, error } = await supabase
      ...
      .select(
        `
        *,
        user:users!messages_sender_id_fkey(id, email, raw_user_meta_data)
      `
      )
      .single()
...
router.post("/chats/:chatId/read", async (req, res) => {
  try {
    ...
    const { error: participantError } = await supabase.from("chat_participants").update({ last_read_at: new Date() }).eq("chat_id", chatId).eq("user_id", userId)
