...
exports.sendMateRequest = async (req, res) => {
  ...
  try {
    ...
    const { data: fromProfile, error: fromErr } = await supabase.from("profiles").select("id, mate_requests").eq("id", fromUserId).single()
    if (fromErr || !fromProfile) return res.status(404).json({ error: "Sender profile not found" })
    // Check for existing request
    const alreadyRequested = [...(toUser.mate_requests || []), ...(fromProfile.mate_requests || [])].some(r => (r.sender === fromUserId && r.receiver === toUser.id && r.status === "pending") || (r.sender === toUser.id && r.receiver === fromUserId && r.status === "pending"))
...
exports.getMateRequests = async (req, res) => {
  const userId = req.user.id
  try {
    const { data: me, error } = await supabase.from("profiles").select("mate_requests").eq("id", userId).single()
    if (error || !me) throw error
    // Filter requests
...
exports.respondToMateRequest = async (req, res) => {
  ...
  try {
    // Get own profile
    const { data: me, error: meErr } = await supabase.from("profiles").select("mate_requests, mates").eq("id", userId).single()
    ...
    await supabase.from("profiles").update({ mate_requests: updatedRequests, mates: newMates }).eq("id", userId)
    // Update mate_requests and mates in the other user's profile
    const otherUserId = sender === userId ? receiver : sender
    const { data: other, error: otherErr } = await supabase.from("profiles").select("mate_requests, mates").eq("id", otherUserId).single()
    ...
    await supabase.from("profiles").update({ mate_requests: updatedOtherRequests, mates: otherMates }).eq("id", otherUserId)
