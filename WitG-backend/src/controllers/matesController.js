...
const sendMateRequest = async (req, res) => {
    ...
    try {
        ...
        const { data: existing } = await supabase
            .from('mates')
            .select('*')
            .or(`(user_one_id.eq.${action_user_id},user_two_id.eq.${recipient_id}),(user_one_id.eq.${recipient_id},user_two_id.eq.${action_user_id})`);
...
const updateMateRequest = async (req, res) => {
    ...
    try {
        ...
        const { data: updatedRequest, error: updateError } = await supabase
            .from('mates')
            .update({ status: status, action_user_id: action_user_id })
            .eq('id', requestId)
            .select()
            .single();
...
const getMyProfile = async (req, res) => {
    ...
    try {
        const { data, error } = await supabase
            .from('profiles')
            .select('id, username, full_name, avatar_url, user_code')
            .eq('id', userId)
            .single();
